...
def main(_argv):
    ...
    feature_params = dict(maxCorners=25, qualityLevel=0.2, minDistance=2, blockSize=7)
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            ###########################
            minx, miny, maxx, maxy = bbox.copy()
            pad_size = 0
            minx, miny, maxx, maxy = max(int(minx - pad_size), 5), max(int(miny - pad_size), 5), int(maxx + pad_size), int(maxy + pad_size)
            ...
            if FLAGS.optical_flow and track.time_since_update <=3:
                ...
                if track.state == 1:
                    track.prev_gray = cv2.cvtColor(frame_cop, cv2.COLOR_BGR2GRAY)
                    ...
                    if track.prev is not None:
                        next, status, error = cv2.calcOpticalFlowPyrLK(track.prev_gray, gray, track.prev, None, **lk_params)
                        # Selects good feature points for previous position
                        good_old = track.prev[status == 1].astype(int)
                        ...
                        for i, (new, old) in enumerate(zip(good_new, good_old)):
                            ...
                            track.mask = cv2.line(track.mask, (a, b), (c, d), color, 2)
                            # Draws filled circle (thickness of -1) at new position with green color and radius of 3
                            frame = cv2.circle(frame, (a, b), 3, color, -1)
                        # Overlays the optical flow tracks on the original frame
                        frame = cv2.add(frame, track.mask)
                        # Updates previous frame
                        track.prev_gray = gray.copy()
                        # Updates previous good feature points
                        track.prev = good_new.reshape(-1, 1, 2)
                        # Opens a new window and displays the output frame
                        # inv_show = cv2.resize(track.mask_gray, (960, 540))  # Resize image
